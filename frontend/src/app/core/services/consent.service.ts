...
//     ApiResponse<{ id: number; consentId: string; consentName: string; deliveryChannels: string[] }[]>
//   > {
//     return this.http
//       .get<
//         ApiResponse<{ id: number; consentId: string; consentName: string; deliveryChannels: string[] }[]>
//       >(`${environment.apiUrl}/consents/workflow-templates`)
//       .pipe(catchError(this.handleError('getWorkflowTemplates')));
//   }
// }
...
export class ConsentService {
  ...
  getMakerStats(): Observable<ApiResponse<MakerStats>> {
    return this.http
      .get<ApiResponse<MakerStats>>(`${environment.apiUrl}/consents/stats/maker`)
      .pipe(catchError(this.handleError('getMakerStats')));
  }

  getCheckerStats(): Observable<ApiResponse<CheckerStats>> {
    return this.http
      .get<ApiResponse<CheckerStats>>(`${environment.apiUrl}/consents/stats/checker`)
      .pipe(catchError(this.handleError('getCheckerStats')));
  }
  ...
  createConsent(data: ConsentCreatePayload): Observable<ApiResponse<Consent>> {
    return this.http.post<ApiResponse<Consent>>(`${environment.apiUrl}/consents`, data).pipe(
      map((res) => (res.data ? { ...res, data: this.normalizeConsent(res.data) } : res)),
      tap((res) => this.logger.debug('ConsentService', `createConsent id=${res.data?.consentId}`)),
      catchError(this.handleError('createConsent')),
    );
  ...
  updateConsent(id: number, data: ConsentCreatePayload): Observable<ApiResponse<Consent>> {
    return this.http.put<ApiResponse<Consent>>(`${environment.apiUrl}/consents/${id}`, data).pipe(
      map((res) => (res.data ? { ...res, data: this.normalizeConsent(res.data) } : res)),
      tap((res) => this.logger.debug('ConsentService', `updateConsent version=${res.data?.version}`)),
      catchError(this.handleError('updateConsent')),
    );
  ...
  saveDraft(id: number, data: ConsentCreatePayload): Observable<ApiResponse<Consent>> {
    const headers = new HttpHeaders().set('X-Skip-Loading', 'true');
    return this.http
      .post<ApiResponse<Consent>>(`${environment.apiUrl}/consents/${id}/save-draft`, data, { headers })
      .pipe(
        map((res) => (res.data ? { ...res, data: this.normalizeConsent(res.data) } : res)),
        tap(() =>
          this.logger.debug(
  ...
  getPendingApprovals(): Observable<ApiResponse<Consent[]>> {
    return this.http.get<ApiResponse<Consent[]>>(`${environment.apiUrl}/consents/pending-approval`).pipe(
      map((res) =>
        res.data
          ? { ...res, data: res.data.map((c) => this.normalizeConsent(c)) }
          : res,
      ),
      tap((res) => this.logger.debug('ConsentService', `getPendingApprovals count=${res.data?.length ?? 0}`)),
      catchError(this.handleError('getPendingApprovals')),
  ...
  reviewConsent(
    ...
    return this.http
      .post<ApiResponse<Consent>>(`${environment.apiUrl}/consents/${id}/review`, { action, reason, revisionMessage })
      .pipe(
        map((res) => (res.data ? { ...res, data: this.normalizeConsent(res.data) } : res)),
        tap((res) =>
          this.logger.debug(
  ...
  getConsentDiff(id: number): Observable<ApiResponse<ConsentDiff>> {
    return this.http.get<ApiResponse<ConsentDiff>>(`${environment.apiUrl}/consents/${id}/diff`).pipe(
      tap((res) =>
        this.logger.debug('ConsentService', `getConsentDiff count=${res.data?.differences?.length ?? 0}`),
      ),
      catchError(this.handleError('getConsentDiff')),
    );
  }
  ...
  getWorkflowTemplates(): Observable<
    ApiResponse<{ id: number; consentId: string; consentName: string; channelEmail: boolean; channelSms: boolean; channelPush: boolean; channelWhatsapp: boolean; channelRcs: boolean }[]>
  > {
    return this.http
      .get<
        ApiResponse<{ id: number; consentId: string; consentName: string; channelEmail: boolean; channelSms: boolean; channelPush: boolean; channelWhatsapp: boolean; channelRcs: boolean }[]>
