...
export class CreateConsentComponent implements OnInit, OnDestroy {
  ...
  readonly form = this.fb.nonNullable.group(
    {
      ...
      category: this.fb.nonNullable.control<ConsentCategory>('MARKETING', Validators.required),
      validityPreset: this.fb.nonNullable.control('365'),
      validityStartDate: this.fb.control<Date | ''>(''),
      validityEndDate: this.fb.control<Date | ''>(''),
      consentMessage: [''],
      channelEmail: this.fb.nonNullable.control(false),
      channelSms: this.fb.nonNullable.control(false),
      channelPush: this.fb.nonNullable.control(false),
  ...
  ngOnInit(): void {
    ...
    this.subs.add(
      this.form.controls.validityPreset.valueChanges.subscribe((preset) => {
        this.applyDateValidators(preset === 'custom');
    ...
    // Set the initial validator state based on the current preset.
    this.applyDateValidators(this.form.controls.validityPreset.value === 'custom');
  ...
  /**
   * Attach the required/past-date validators only when the custom preset is active.
   * For fixed presets (15/30/.../365/730) the date controls stay empty and must not
   * be required, otherwise the whole form stays invalid and Submit is blocked.
   */
  private applyDateValidators(custom: boolean): void {
    const start = this.form.controls.validityStartDate;
    const end = this.form.controls.validityEndDate;

    if (custom) {
      start.setValidators([Validators.required, notPastDateValidator()]);
      end.setValidators([Validators.required, notPastDateValidator()]);
    } else {
            ...
            this.api.submitForApproval(id).subscribe({
              next: (res) => {
                if (res.success && res.data) {
                  this.loadedStatus.set(res.data.status);
                  this.notify.success('Submitted', res.message || 'Submitted for approval.');
                  void this.router.navigate(['/maker/consents'], { replaceUrl: true });
                }
              },
            }),
          );
