...
var ConsentService = class _ConsentService {
  ...
  normalizeConsent(c) {
    return __spreadProps(__spreadValues({}, c), {
      tagsUsed: Array.isArray(c.tagsUsed) ? [...c.tagsUsed] : []
    });
  }
  mapPaged(res) {
    if (!res.data?.content) {
  ...
  createConsent(data) {
    return this.http.post(`${environment.apiUrl}/consents`, data).pipe(map((res) => res.data ? __spreadProps(__spreadValues({}, res), { data: this.normalizeConsent(res.data) }) : res), tap((res) => this.logger.debug("ConsentService", `createConsent id=${res.data?.consentId}`)), catchError(this.handleError("createConsent")));
  }
  updateConsent(id, data) {
    return this.http.put(`${environment.apiUrl}/consents/${id}`, data).pipe(map((res) => res.data ? __spreadProps(__spreadValues({}, res), { data: this.normalizeConsent(res.data) }) : res), tap((res) => this.logger.debug("ConsentService", `updateConsent version=${res.data?.version}`)), catchError(this.handleError("updateConsent")));
  }
  deleteConsent(id) {
  ...
  saveDraft(id, data) {
    ...
    return this.http.post(`${environment.apiUrl}/consents/${id}/save-draft`, data, { headers }).pipe(map((res) => res.data ? __spreadProps(__spreadValues({}, res), { data: this.normalizeConsent(res.data) }) : res), tap(() => this.logger.debug("ConsentService", `saveDraft Auto-saved at ${(/* @__PURE__ */ new Date()).toLocaleTimeString("en-IN", { hour12: false })}`)), catchError(this.handleError("saveDraft")));
  }
  ...
  getPendingApprovals() {
    return this.http.get(`${environment.apiUrl}/consents/pending-approval`).pipe(map((res) => res.data ? __spreadProps(__spreadValues({}, res), { data: res.data.map((c) => this.normalizeConsent(c)) }) : res), tap((res) => this.logger.debug("ConsentService", `getPendingApprovals count=${res.data?.length ?? 0}`)), catchError(this.handleError("getPendingApprovals")));
  }
  publishConsent(id) {
    return this.reviewConsent(id, "PUBLISH");
  }
  reviewConsent(id, action, reason, revisionMessage) {
    return this.http.post(`${environment.apiUrl}/consents/${id}/review`, { action, reason, revisionMessage }).pipe(map((res) => res.data ? __spreadProps(__spreadValues({}, res), { data: this.normalizeConsent(res.data) }) : res), tap((res) => this.logger.debug("ConsentService", `reviewConsent Action ${action} \u2192 status ${res.data?.status ?? "unknown"}`)), catchError(this.handleError("reviewConsent")));
  }
  getConsentDiff(id) {
  ...
  getWorkflowTemplates() {
    return this.http.get(`${environment.apiUrl}/consents/workflow-templates`).pipe(catchError(this.handleError("getWorkflowTemplates")));
  }
  static {
...
//# sourceMappingURL=chunk-CSLDYKDC.js.map
