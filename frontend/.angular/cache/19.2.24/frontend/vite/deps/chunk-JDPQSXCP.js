...
var Mark = class _Mark2 {
  ...
  */
  static setFrom(marks) {
    if (!marks || Array.isArray(marks) && marks.length == 0) return _Mark2.none;
    if (marks instanceof _Mark2) return [marks];
...
function removeRange(content, from, to) {
  let {
    index,
    offset
  } = content.findIndex(from), child = content.maybeChild(index);
  let {
    index: indexTo,
    offset: offsetTo
...
function renderSpec(doc3, structure, xmlNS, blockArraysIn) {
  ...
  for (let i = start; i < structure.length; i++) {
    ...
    if (child === 0) {
      ...
      } = renderSpec(doc3, child, xmlNS, blockArraysIn);
...
var ReplaceStep = class _ReplaceStep extends Step {
  ...
  */
  ...
  apply(doc3) {
    if (this.structure && contentBetween(doc3, this.from, this.to)) return StepResult.fail("Structure replace would overwrite content");
...
var ReplaceAroundStep = class _ReplaceAroundStep extends Step {
  ...
  */
  ...
  apply(doc3) {
    if (this.structure && (contentBetween(doc3, this.from, this.gapFrom) || contentBetween(doc3, this.gapTo, this.to))) return StepResult.fail("Structure gap-replace would overwrite content");
...
function joinPoint(doc3, pos, dir = -1) {
  ...
  for (let d = $pos.depth; ; d--) {
    ...
    if (before && !before.isTextblock && joinable2(before, after) && $pos.node(d).canReplace(index, index + 1)) return pos;
    if (d == 0) break;
...
function wrapRangeInList(tr, range, listType, attrs = null) {
  ...
  if (range.depth >= 2 && range.$from.node(range.depth - 1).type.compatibleContent(listType) && range.startIndex == 0) {
    ...
    if (range.endIndex < range.parent.childCount) range = new NodeRange(range.$from, doc3.resolve(range.$to.end(range.depth)), range.depth);
    doJoin = true;
  }
...
var ViewDesc = class {
  ...
  domFromPos(pos, side) {
    ...
    if (side <= 0) {
      ...
      if (prev && side && enter2 && !prev.border && !prev.domAtom) return prev.domFromPos(prev.size, side);
      return {
      ...
      if (next && enter2 && !next.border && !next.domAtom) return next.domFromPos(0, side);
      return {
        node: this.contentDOM,
...
function findTextInFragment(frag, text, from, to) {
  for (let i = 0, pos = 0; i < frag.childCount && pos <= to; ) {
    ...
    if (pos >= from) {
      ...
      if (found2 >= 0 && found2 + text.length + childStart >= from) return childStart + found2;
...
function withWrappers(node, wrap2, from = 0) {
  for (let i = wrap2.length - 1; i >= from; i--) node = wrap2[i].create(null, Fragment.from(node));
  return node;
}
...
function defaultTripleClick(view, inside, event) {
  ...
  for (let i = $pos.depth + 1; i > 0; i--) {
    ...
    if (node.inlineContent) updateSelection(view, TextSelection.create(doc3, nodePos + 1, nodePos + 1 + node.content.size), "pointer");
    else if (NodeSelection.isSelectable(node)) updateSelection(view, NodeSelection.create(doc3, nodePos), "pointer");
    else continue;
    return true;
  }
}
...
var DecorationSet = class _DecorationSet {
  ...
  */
  ...
  addInner(doc3, decorations, offset) {
    ...
    for (let i = 0; i < local.length; i++) if (!local[i].type.valid(doc3, local[i])) local.splice(i--, 1);
    ...
    if (((_a = node.parentNode) === null || _a === void 0 ? void 0 : _a.nodeName) == "TR") {
      ...
      if (nextCell) {
        ...
        for (; ; ) {
          ...
          if (!first2 || first2.nodeType != 1 || first2.contentEditable == "false" || /^(BR|IMG)$/.test(first2.nodeName)) break;
          parent = first2;
        }
...
function normalize(map) {
  ...
  for (let prop in map) copy2[normalizeKeyName(prop)] = map[prop];
  return copy2;
}
...
var CommandManager = class {
  ...
  createCan(startTr) {
    ...
    const formattedCommands = Object.fromEntries(Object.entries(rawCommands).map(([name, command2]) => {
      return [name, (...args) => command2(...args)(__spreadProps(__spreadValues({}, props), {
        dispatch: void 0
      }))];
    }));
...
var forEach = (items, fn) => (props) => {
  return items.every((item, index) => fn(item, __spreadProps(__spreadValues({}, props), {
    index
  })));
};
...
var joinTextblockBackward2 = () => ({
  state,
  dispatch
}) => {
  return joinTextblockBackward(state, dispatch);
};
var joinTextblockForward2 = () => ({
  state,
  dispatch
}) => {
...
var lift3 = (typeOrName, attributes = {}) => ({
  state,
  dispatch
}) => {
  const type = getNodeType(typeOrName, state.schema);
...
function generateText(doc3, extensions, options) {
  ...
  return getText2(contentNode, {
    blockSeparator,
    textSerializers: __spreadValues(__spreadValues({}, getTextSerializersFromSchema(schema)), textSerializers)
  });
}
...
function isNodeViewSelected({
  ...
  if (from <= pos && to >= pos + nodeSize2) {
    return true;
  }
  if (selectedOnTextSelection && isTextSelection(selection) && from > pos && to < pos + nodeSize2) {
    return true;
  }
  return false;
}
...
var toggleNode = (typeOrName, toggleTypeOrName, attributes = {}) => ({
  ...
  return commands.setNode(type, __spreadValues(__spreadValues({}, attributesToCopy), attributes));
};
...
var unsetTextDirection = (position) => ({
  ...
  if (dispatch) {
    tr.doc.nodesBetween(from, to, (node, pos) => {
      ...
      const newAttrs = __spreadValues({}, node.attrs);
      delete newAttrs.dir;
...
var wrapIn2 = (typeOrName, attributes = {}) => ({
  state,
  dispatch
}) => {
  const type = getNodeType(typeOrName, state.schema);
...
var wrapInList2 = (typeOrName, attributes = {}) => ({
  state,
  dispatch
}) => {
  const type = getNodeType(typeOrName, state.schema);
...
var Extendable = class {
  ...
  get options() {
    return __spreadValues({}, callOrReturn(getExtensionField(this, "addOptions", {
      name: this.name
    })) || {});
  }
  get storage() {
    return __spreadValues({}, callOrReturn(getExtensionField(this, "addStorage", {
      name: this.name,
  ...
  extend(extendedConfig = {}) {
    const extension = new this.constructor(__spreadValues(__spreadValues({}, this.config), extendedConfig));
  ...
  get plugins() {
    ...
    const allPlugins = extensions.flatMap((extension) => {
      ...
      if (extension.type === "mark" && getExtensionField(extension, "exitable", context)) {
        defaultBindings.ArrowRight = () => Mark2.handleExit({
          editor,
          mark: extension
        });
      }
    ...
    return [new Plugin({
      ...
      props: {
        clipboardTextSerializer: () => {
          ...
          return getTextBetween(doc3, range, __spreadProps(__spreadValues({}, this.options.blockSeparator !== void 0 ? {
...
var Commands = Extension.create({
  name: "commands",
  addCommands() {
    return __spreadValues({}, commands_exports);
  }
});
    ...
    return [new Plugin({
      ...
      props: {
        attributes: () => {
          ...
          return {
            tabindex: (_a = this.options.value) != null ? _a : "0"
          };
        }
      }
    })];
...
var Editor = class extends EventEmitter {
  ...
  createView(element) {
    ...
    this.editorView = new EditorView(element, __spreadProps(__spreadValues({}, editorProps), {
...
var ResizableNodeView = class {
  ...
  constructor(options) {
    ...
    if ((_a = options.options) == null ? void 0 : _a.min) {
      this.minSize = __spreadValues(__spreadValues({}, this.minSize), options.options.min);
    }
...
function createAtomBlockMarkdownSpec(options) {
  ...
  return {
    parseMarkdown: (token, h2) => {
      const attrs = __spreadValues(__spreadValues({}, defaultAttributes), token.attributes);
...
function updateMarkViewAttributes(checkMark, editor, attrs = {}) {
  ...
  doc3.descendants((node, pos) => {
    ...
    if (needsUpdate) {
      const updatedMark = checkMark.type.create(__spreadValues(__spreadValues({}, checkMark.attrs), attrs));
...
//# sourceMappingURL=chunk-JDPQSXCP.js.map
