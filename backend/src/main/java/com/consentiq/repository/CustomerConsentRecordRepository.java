...
public interface CustomerConsentRecordRepository extends JpaRepository<CustomerConsentRecord, Long> {
	...
	// Customers whose LATEST consent record (by id) has respondedAt set
	@Query("""
	    SELECT COUNT(DISTINCT r.customerId)
	    FROM CustomerConsentRecord r
	    WHERE r.id IN (
	    ...
	    AND r.respondedAt IS NOT NULL
	""")
	long countDistinctRespondedCustomers();
	...
	@Query("""
	    ...
	    AND r.respondedAt IS NOT NULL
	""")
	List<String> findLatestRespondedCustomerIds();
}